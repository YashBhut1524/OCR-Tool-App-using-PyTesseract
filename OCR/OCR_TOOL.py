...
class MainApp(QtWidgets.QMainWindow):
    ...
    def extract_text_with_format(self, image):
        ...
        text = pytesseract.image_to_string(gray, lang=self.get_selected_language(), config=custom_config)
        return text
    ...
    def toggle_dark_mode(self):
        dark_mode_stylesheet = "background-color: #2E2E2E; color: white;"
        light_mode_stylesheet = "background-color: white; color: black;"
        self.central_widget.setStyleSheet(dark_mode_stylesheet if self.central_widget.styleSheet() != dark_mode_stylesheet else light_mode_stylesheet)

    def update_word_count(self):
        word_count = len(self.text_display.toPlainText().split())
